...
const TABS = ["properties", "layers", "quantities", "survey", "ai"] as const;
  ...
  return (
    <div className="space-y-3">
      ...
      <Field
        label="Name"
        value={entity.name ?? ""}
        onChange={(v) => update(entity.id, { name: v } as Partial<Entity>)}
      />
...
function EntityFields({
    ...
    return (
      <>
        ...
        <Select
          ...
          onChange={(v) => onChange({ material: v } as Partial<Entity>)}
        />
  ...
  if (e.kind === "door" || e.kind === "window") {
    return (
      <>
        ...
        {e.kind === "window" && (
          <Num label="Sill" mm={e.sill} units={units} onMm={(v) => onChange({ sill: v } as Partial<Entity>)} />
        )}
  ...
  if (e.kind === "circle") {
    return <Num label="Radius" mm={e.r} units={units} onMm={(v) => onChange({ r: v } as Partial<Entity>)} />;
  }
  if (e.kind === "rect") {
    return (
      <>
  ...
  if (e.kind === "text") {
    return <Field label="Text" value={e.text} onChange={(v) => onChange({ text: v } as Partial<Entity>)} />;
  }
  if (e.kind === "column") {
    return (
      <>
    ...
    <div className="space-y-3">
      {rows.map((r) => {
        ...
        return (
          <div key={r.id}>
            {head && (
              <div className="mb-1 font-mono text-[10px] tracking-widest text-subtle uppercase">{r.group}</div>
            )}
...
function Survey() {
  ...
  if (!points.length) {
    return <p className="text-sm text-muted">No survey stations. Use the Station tool or ask AI to place control.</p>;
  }
      ...
      <div>
        ...
        {inv && (
          <div className="mt-2 space-y-1 font-mono text-xs">
            ...
            <Meta label="ΔH" value={formatMm(inv.dH, units)} />
          </div>
        )}
      </div>
    </div>
...
const AI_HINTS = [
  "Draw a 10 × 6 m room with 200 mm brick walls and a 900 mm door on the south side",
  "Add aligned dimensions to the exterior walls",
  "Place a survey station at each building corner",
  "Four Ø8 holes, 15 mm in from each plate corner",
];
  ...
  return (
    <div className="flex h-full min-h-[240px] flex-col gap-3">
      <p className="text-sm text-muted">
        Describe a room, a plate, a traverse. Coordinates land in millimetres; the model stays parametric.
      </p>
        ...
        {log.length === 0 &&
          AI_HINTS.map((h) => (
            <button
              ...
              type="button"
              onClick={() => send(h)}
              className="rounded-sm bg-elevated px-2.5 py-2 text-left text-xs text-muted transition-colors hover:text-fg"
            >
              {h}
            </button>
          ))}
      ...
      >
        <input
          ...
          placeholder="Instruct the model…"
          className="h-10 min-w-0 flex-1 rounded-sm bg-elevated px-2.5 text-sm outline-none placeholder:text-subtle"
        />
        <button
          type="submit"
          disabled={busy || !prompt.trim()}
          className="inline-flex size-10 items-center justify-center rounded-sm bg-primary text-primary-fg disabled:opacity-40"
          aria-label="Send"
        >
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className="h-9 w-full rounded-sm bg-elevated px-2 text-sm outline-none"
      />
    </label>
  );
}
...
function Num({
  ...
  return (
    <label className="block">
      <span className="mb-1 block text-[10px] font-medium tracking-wide text-subtle uppercase">
        {label} ({units})
      </span>
      <input
        defaultValue={shown}
